...
resource "aws_iam_role" "this" {
  name               = local.name
  assume_role_policy = var.external_id == "" ? data.aws_iam_policy_document.role.json : data.aws_iam_policy_document.role_external_id.json
  tags = {
    identifiers = var.principal_arns
    external_id = var.external_id != "" ? "true" : "false"
  }
}
